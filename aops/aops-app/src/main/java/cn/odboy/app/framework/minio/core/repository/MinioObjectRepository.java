...
import cn.odboy.app.framework.minio.core.vo.MinioStorageVo;
...
public class MinioObjectRepository {
    ...
    public List<MinioStorageVo> describeObjectListByRegionWithBucketName(String region, String bucketName) {
        List<MinioStorageVo> result = new ArrayList<>();
        try (MinioClient client = minioClientAdmin.getClient()) {
            ...
            for (Result<Item> iterable : iterables) {
                ...
                MinioStorageVo record = new MinioStorageVo();
                record.setFileName(item.objectName());
                String suffix = FileUtil.getSuffix(item.objectName());
                record.setSuffix(suffix);
                record.setFileType(cn.odboy.common.util.FileUtil.getFileType(suffix));
    ...
    public List<MinioStorageVo> describeDefaultRegionObjectListByBucketName(String bucketName) {
